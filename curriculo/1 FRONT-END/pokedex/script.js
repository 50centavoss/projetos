...
function buscar(){
  ...
  .then(data => {
    ...
    tela.innerHTML = 
    '<h2>'+ data.name +'</h2>'
    +' <img class="pokemon1" src="https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/versions/generation-v/black-white/animated/'+data.id+'.gif" > '
    + ' <img class="pokemon" src="'+ data.sprites.front_default+'" >'
    + ' <p> Id: '+ data.id + '</p> '
    + ' <p> Tipo:'+ data.types.map(type => type.type.name) + '</p>'
    + ' <p> Habilidade: '+data.abilities.map(ability => ability.ability.name)
